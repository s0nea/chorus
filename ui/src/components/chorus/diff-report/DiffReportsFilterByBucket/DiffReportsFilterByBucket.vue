...
<script setup lang="ts">
  ...
  const { reports, filterBuckets, page } = storeToRefs(
    useChorusDiffReportsStore(),
  );

  const bucketPairOptions = computed(() =>
    reports.value.map((report) => DiffReportsHelper.getBucketPair(report)),
  );
</script>
