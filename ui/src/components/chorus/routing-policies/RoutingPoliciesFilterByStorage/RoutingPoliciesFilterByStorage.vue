...
<script setup lang="ts">
  ...
  const { userRoutingPolicies, bucketRoutingPolicies, filterStorages, page } =
  ...
  >(() => {
    const sortedUniqueStorage = [
      ...new Set(
        userRoutingPolicies.value.map(
          (userRoutingPolicy) => userRoutingPolicy.toStorage,
        ),
      ),
      ...new Set(
        bucketRoutingPolicies.value.map(
          (bucketRoutingPolicy) => bucketRoutingPolicy.toStorage,
        ),
      ),
    ].sort();
