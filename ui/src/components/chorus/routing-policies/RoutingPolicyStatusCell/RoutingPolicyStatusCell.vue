...
<script setup lang="ts">
  ...
  import type { ChorusRoutingPolicy } from '@/utils/types/chorus';
  ...
  defineProps<{
    routingPolicy: ChorusRoutingPolicy;
  }>();
</script>
    ...
    <CTooltip :delay="500">
      <template #trigger>
        <CTag
          ...
          round
          size="small"
          class="routing-policy-storage-cell__status"
      ...
      <span class="routing-policy-storage-cell__status-tooltip">
        <strong>{{
          routingPolicy.isBlocked ? t('statusBlocked') : t('statusAllowed')
        }}</strong
        >:
